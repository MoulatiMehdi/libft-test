...
void	*strlcpy_wrapper(void *args)
{
	...
	params = args;
	(strlcpy(params->dst, params->src, params->dstsize));
	return (0);
}
...
void	*ft_strlcpy_wrapper(void *args)
{
	...
	params = args;
	(ft_strlcpy(params->dst, params->src, params->dstsize));
	return (0);
}
		...
		for (j = 0; j < strs_len; j++)
		{
			...
			for (k = 0; k < sizes_len; k++)
			{
				...
				if (exp_status == res_status && exp_status == 0)
				{
					...
					res = ft_strlcpy(res_args.dst, res_args.src,
							res_args.dstsize);
					exp = strlcpy(exp_args.dst, exp_args.src, exp_args.dstsize);
					if (res != exp)
					{
						sprintf(nb1, "%lu", exp);
						sprintf(nb2, "%lu", res);
